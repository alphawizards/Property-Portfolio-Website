...
## Fix portfolio.getGoal Query Error (Completed)

- [x] Investigate portfolio.getGoal backend procedure implementation
- [x] Check if portfolio goal data exists in database for user
- [x] Fix query to return null instead of undefined when no goal is set
- [x] Frontend already handles null/undefined goal data gracefully with ternary operator
- [x] Test fix and verify error is resolved (Goal Reached card shows "Not Set" without errors)

## Portfolio Goal Setting Modal (Completed)

- [x] Create Dialog component for goal setting modal with DialogTrigger wrapping card
- [x] Add form fields for target year (number input with min validation)
- [x] Add form field for desired equity amount (thousands input with helpful placeholder)
- [x] Make Portfolio Goal card clickable to open modal (cursor-pointer with hover effect)
- [x] Connect form to portfolio.setGoal mutation with automatic query invalidation
- [x] Update Portfolio Goal card to display set goal (FY2035 + Target: $5000k equity)
- [x] Add validation for year (must be > current year) and equity (must be positive)
- [x] Test goal setting, saving, and display (tested with FY2035 and $5M target)

## Detailed Cashflow Projection Table (Completed)

- [x] Design cashflow table structure matching Zapiio layout (10-year columns, categorized rows)
- [x] Create backend calculation functions for detailed cashflow breakdown (generateDetailedCashflowTable)
- [x] Calculate year-by-year rental income with growth rate applied
- [x] Calculate year-by-year interest and principal payments from loan data
- [x] Calculate year-by-year expenses with growth rate applied (breakdown by category)
- [x] Add depreciation calculations from depreciation schedule
- [x] Add tax calculations (Tax Credit, Tax Debit, Gross Before Tax, Net After Tax)
- [x] Create tRPC procedure to return detailed cashflow data (detailedCashflowTable)
- [x] Implement collapsible table UI component with toggle button (DetailedCashflowTable)
- [x] Add property selector integration (All Properties vs individual property via propertyId parameter)
- [x] Style table to match Zapiio design (blue header, categorized sections, green highlighted totals)
- [x] Table supports horizontal scrolling for many years with sticky first column
- [x] Integrated into Dashboard below equity chart, appears only when properties exist

## Australian Payroll and Tax Calculator (In Progress)

### Phase 1: Data Model and API Design
- [ ] Design income_profiles table schema (base income, pay frequency, residency status)
- [ ] Design superannuation_settings table (guarantee rate, salary sacrifice, caps)
- [ ] Design tax_settings table (tax-free threshold, student loan, Medicare exemptions)
- [ ] Design family_tax_benefit table (children details, shared care, child support)
- [ ] Design deductions table (work expenses, novated lease, other deductions)
- [ ] Design tax_calculations_cache table (store calculated results for performance)
- [ ] Define tRPC API endpoints for income CRUD operations
- [ ] Define tRPC API endpoint for tax calculation (calculate.payrollTax)
- [ ] Define tRPC API endpoint for what-if scenarios (calculate.taxScenario)
- [ ] Document data model relationships and constraints

### Phase 2: Database Schema Implementation
- [ ] Create income_profiles table with all required fields
- [ ] Create superannuation_settings table with validation constraints
- [ ] Create tax_settings table with enum types for residency status
- [ ] Create family_tax_benefit table with child age validation
- [ ] Create deductions table with category enum
- [ ] Add foreign key relationships to user table
- [ ] Run database migration (pnpm db:push)
- [ ] Create seed data for testing

### Phase 3: Backend Tax Calculation Engine
- [ ] Implement ATO tax bracket calculations for 2025-26 (resident rates)
- [ ] Implement Medicare levy calculation (2% with phase-in thresholds)
- [ ] Implement Medicare levy surcharge (1-1.5% based on income/insurance)
- [ ] Implement student loan repayment calculation (HELP/HECS marginal system)
- [ ] Implement LITO (Low Income Tax Offset) calculation
- [ ] Implement seniors/pensioners offset calculation
- [ ] Implement superannuation guarantee calculation (12% from July 2025)
- [ ] Implement salary sacrifice and concessional cap logic
- [ ] Implement tax-free threshold vs no-threshold calculations
- [ ] Implement non-resident and WHM tax scales
- [ ] Implement pay frequency proration (weekly/fortnightly/monthly/annual)
- [ ] Implement Family Tax Benefit estimation
- [ ] Create comprehensive tax calculation function (calculateAustralianTax)
- [ ] Add support for historical tax years (2020-21 to 2025-26)
- [ ] Write unit tests for all tax calculation functions

### Phase 4: Frontend UI Implementation
- [ ] Create Income Profile page/modal for data entry
- [ ] Add gross income input with pay frequency selector
- [ ] Add residency status selector (resident/non-resident/WHM)
- [ ] Add superannuation settings section (plus/inclusive, sacrifice, caps)
- [ ] Add tax-free threshold toggle
- [ ] Add student loan debt input with threshold display
- [ ] Add overtime/bonus/part-time hours inputs
- [ ] Add deductions section (work expenses, novated lease)
- [ ] Add Medicare exemption and private health insurance inputs
- [ ] Add Family Tax Benefit section (children details)
- [ ] Create Tax Summary component showing breakdown by pay frequency
- [ ] Display gross pay, tax, Medicare, student loan, super, net pay table
- [ ] Add detailed breakdown sections (income, super, tax, offsets, FTB)
- [ ] Add income percentile comparison visualization
- [ ] Add what-if scenario adjustments (salary slider, year selector)
- [ ] Add export functionality (PDF/CSV)

### Phase 5: Portfolio Integration
- [ ] Add rental income to taxable income calculations
- [ ] Calculate combined tax position (salary + rental income + expenses)
- [ ] Show tax deductions from property expenses (interest, depreciation)
- [ ] Display marginal tax rate impact of rental income
- [ ] Create combined cashflow view (salary + rental income - all taxes)
- [ ] Add scenario comparison (with/without property income)
- [ ] Update dashboard to show after-tax portfolio returns

### Phase 6: Testing and Validation
- [ ] Test all tax brackets with sample incomes ($0, $50k, $100k, $200k)
- [ ] Verify Medicare levy calculations with phase-in thresholds
- [ ] Test student loan repayments at various income levels
- [ ] Validate superannuation calculations and caps
- [ ] Test LITO and other offsets application
- [ ] Verify pay frequency proration accuracy
- [ ] Test non-resident and WHM tax scales
- [ ] Validate Family Tax Benefit calculations
- [ ] Test integration with property portfolio tax calculations
- [ ] Save checkpoint and document all formulas


## Australian Payroll and Tax Calculator (IN PROGRESS)

### Phase 1: Design & Documentation (COMPLETED)
- [x] Design data model for income profiles and tax settings
- [x] Create SQL DDL scripts for tax_brackets and medicare_levy_thresholds
- [x] Create SQL DDL script for HELP/HECS repayment brackets (19 brackets)
- [x] Document detailed calculation logic for Medicare levy and HELP repayments
- [x] Define tRPC API endpoints specification
- [x] Create comprehensive implementation plan

### Phase 2: Database Schema Implementation (COMPLETED)
- [x] Add tax_brackets table to Drizzle schema
- [x] Add medicare_levy_thresholds table to Drizzle schema
- [x] Add help_repayment_brackets table to Drizzle schema
- [x] Add tax_offsets table to Drizzle schema
- [x] Add superannuation_rates table to Drizzle schema
- [x] Add income_profiles table to Drizzle schema
- [x] Add superannuation_settings table to Drizzle schema
- [x] Add tax_settings table to Drizzle schema
- [x] Run database migration (pnpm db:push)
- [x] Create seed script for reference data (2025-26 tax year)
- [x] Execute seed script to populate tax rules

### Phase 3: Backend Tax Calculation Engine (COMPLETED)
- [x] Create server/tax-calculations.ts with calculation functions
- [x] Implement calculateIncomeTax function (all residency types)
- [x] Implement calculateMedicareLevy function with phase-in
- [x] Implement calculateMedicareLevySurcharge function
- [x] Implement calculateHELPRepayment function (marginal system)
- [x] Implement calculateLITO function (Low Income Tax Offset)
- [x] Implement calculateSAPTO function (Seniors offset)
- [x] Implement calculateSuperannuation function (12% guarantee)
- [x] Implement calculateAdjustedTaxableIncome function
- [x] Implement main calculateTax function (orchestrates all calculations)
- [x] Add database helpers in server/db.ts for tax data
- [x] Create income router in server/routers.ts (CRUD operations)
- [x] Create tax router in server/routers.ts (calculate, scenario)
- [x] Write unit tests for all calculation functions (7/7 passing)

### Phase 4: Frontend UI Components (COMPLETED)
- [x] Create TaxCalculatorSection component (collapsible like DetailedCashflowTable)
- [x] Create IncomeProfileForm component (gross income, residency, pay frequency)
- [x] Create SuperannuationSettings component (guarantee, salary sacrifice)
- [x] Create TaxSettings component (tax-free threshold, student loan, Medicare)
- [x] Create TaxSummaryCards component (gross, tax, Medicare, HELP, net pay)
- [x] Create DetailedTaxBreakdown component (income, super, tax, offsets)
- [x] Add toggle button to Dashboard to show/hide tax calculator
- [x] Integrate tax calculator section into Dashboard page
- [x] Add loading states and error handling
- [x] Style components with Tailwind CSS matching existing design

### Phase 5: Property Portfolio Integration
- [ ] Add rental income to taxable income calculations
- [ ] Calculate property expense deductions (interest, depreciation)
- [ ] Show marginal tax rate impact of rental income
- [ ] Create combined tax view (salary + rental income)
- [ ] Add scenario comparison (with/without property income)
- [ ] Display after-tax portfolio returns in dashboard
- [ ] Update cashflow projections to include income tax

### Phase 6: Testing & Validation (COMPLETED)
- [x] Test tax calculations with $60k, $100k incomes (all passing)
- [x] Verify Medicare levy phase-in calculations (working correctly)
- [x] Test HELP repayment calculations at various income levels (marginal system working)
- [x] Validate superannuation calculations and caps (12% guarantee correct)
- [x] Test LITO and SAPTO offset application (offsets applying correctly)
- [x] Test non-resident and WHM tax scales (no Medicare levy for non-residents)
- [x] Test salary sacrifice functionality (reduces taxable income)
- [x] Save checkpoint with complete tax calculator implementation


## User Management & Subscription System

### Phase 1: Architecture & Planning
- [ ] Design user role hierarchy (Admin, Pro, Basic)
- [ ] Define feature access matrix for each tier
- [ ] Design subscription tier table schema
- [ ] Design user subscription relationship schema
- [ ] Document authorization flow and middleware architecture
- [ ] Create feature gate specification document

### Phase 2: Database Schema Extension
- [ ] Add subscription_tiers table (tier name, features, limits)
- [ ] Add user_subscriptions table (user_id, tier_id, start_date, end_date)
- [ ] Update users table with role field (admin, user)
- [ ] Add property_count_limit field to subscription_tiers
- [ ] Add forecast_years_limit field to subscription_tiers
- [ ] Seed subscription tiers (Basic: 2 properties, 10 years; Pro: unlimited)
- [ ] Run database migration (pnpm db:push)

### Phase 3: Backend Authorization
- [ ] Create authorization middleware (checkRole, checkSubscription)
- [ ] Create feature gate helpers (canAddProperty, canViewForecast)
- [ ] Add adminProcedure to tRPC (requires admin role)
- [ ] Add subscription validation to property creation
- [ ] Add forecast year limit validation to calculations
- [ ] Create user management tRPC router (admin only)
- [ ] Write unit tests for authorization logic

### Phase 4: Admin Dashboard
- [ ] Create AdminDashboard page component
- [ ] Create UserManagementTable component (list all users)
- [ ] Create UserRoleEditor component (change user role)
- [ ] Create SubscriptionEditor component (change user tier)
- [ ] Add admin navigation link (visible only to admins)
- [ ] Implement user search and filtering
- [ ] Add user activity statistics

### Phase 5: Feature Restrictions & Upgrade Prompts
- [ ] Add property count check before "Add Property" action
- [ ] Add forecast year limit to year selector dropdown
- [ ] Create UpgradePromptModal component
- [ ] Add upgrade prompts to restricted features (30+ years, 3+ properties)
- [ ] Disable tax calculator for basic users with upgrade prompt
- [ ] Disable investment comparison for basic users
- [ ] Add "Upgrade to Pro" button in navigation
- [ ] Create subscription comparison table

### Phase 6: Testing & Validation
- [ ] Test admin account creation and permissions
- [ ] Test basic user property limit (2 properties max)
- [ ] Test basic user forecast limit (10 years max)
