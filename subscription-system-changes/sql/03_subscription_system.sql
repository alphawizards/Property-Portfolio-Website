...
CREATE TABLE IF NOT EXISTS `subscription_tiers` (
  ...
  `description` text,
  `maxProperties` int NOT NULL DEFAULT 0, -- 0 = unlimited
...
-- Update users table to add role field if it doesn't exist
ALTER TABLE `users` 
  ADD COLUMN IF NOT EXISTS `role` enum('admin', 'user') NOT NULL DEFAULT 'user',
  ADD COLUMN IF NOT EXISTS `isActive` boolean NOT NULL DEFAULT true;

-- Create index for faster lookups
CREATE INDEX IF NOT EXISTS `idx_user_subscriptions_tierId` ON `user_subscriptions`(`tierId`);
...
(`name`, `displayName`, `description`, `maxProperties`, `maxForecastYears`, `canUseAdvancedAnalytics`, `canUseScenarioComparison`, `canExportReports`, `canUseTaxCalculator`, `priceMonthly`, `priceYearly`)
VALUES 
